    ...
    fastify.get('/life/:userID', async (request, reply) => {
        ...
        if (!auth) return reply.status(403).send({
            ...
            if (!user) return reply.status(404).send({
                ...
                message: 'Conflict Found: Provided User is not subscribed to our Lifetime Premium Program',
                error: true,
                fatal: false,
                status: 409
            });
                ...
                return reply.status(200).send({
                    message: 'Okay, That user has been removed from our Lifetime Premium Program!',
                    error: false,
                    fatal: false,
                    status: 200
                });
