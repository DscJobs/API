    ...
    fastify.get('/reg/:userID', async (request, reply) => {
        ...
        if (!auth) return reply.status(403).send({
            ...
            if (!user) return reply.status(404).send({
                ...
                message: 'Conflict Found: Provided User is already subscribed to our Premium Program',
                error: true,
                fatal: false,
                status: 409
            });
                ...
                return reply.status(200).send({
                    message: 'Okay, That user has been added to our Premium Program without a Lifetime Subscription',
                    error: false,
                    fatal: false,
                    status: 200
                });
