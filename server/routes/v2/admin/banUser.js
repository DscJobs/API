...
module.exports = async (fastify, opts) => {
    ...
    fastify.get('/ban/:userID', async (request, reply) => {
        ...
        if (!auth) return reply.status(403).send({
            ...
            if (!user) return reply.status(404).send({
            ...
            else if (user.banned) return reply.status(409).send({
                ...
                user.notifications.set("You have been banned!", "One of our Admins has banned you from accessing our Website. Please join our Discord Server to learn more info");
